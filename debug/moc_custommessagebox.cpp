/****************************************************************************
** Meta object code from reading C++ file 'custommessagebox.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.7.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../custommessagebox.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'custommessagebox.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.7.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_CustomMessageBox_t {
    QByteArrayData data[8];
    char stringdata0[89];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_CustomMessageBox_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_CustomMessageBox_t qt_meta_stringdata_CustomMessageBox = {
    {
QT_MOC_LITERAL(0, 0, 16), // "CustomMessageBox"
QT_MOC_LITERAL(1, 17, 11), // "SendCurType"
QT_MOC_LITERAL(2, 29, 0), // ""
QT_MOC_LITERAL(3, 30, 6), // "nIndex"
QT_MOC_LITERAL(4, 37, 15), // "OnSelectDecArea"
QT_MOC_LITERAL(5, 53, 6), // "bClick"
QT_MOC_LITERAL(6, 60, 15), // "OnSelectTriLine"
QT_MOC_LITERAL(7, 76, 12) // "OnSelectEdit"

    },
    "CustomMessageBox\0SendCurType\0\0nIndex\0"
    "OnSelectDecArea\0bClick\0OnSelectTriLine\0"
    "OnSelectEdit"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_CustomMessageBox[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   34,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       4,    1,   37,    2, 0x08 /* Private */,
       6,    1,   40,    2, 0x08 /* Private */,
       7,    1,   43,    2, 0x08 /* Private */,

 // signals: parameters
    QMetaType::Void, QMetaType::Int,    3,

 // slots: parameters
    QMetaType::Void, QMetaType::Bool,    5,
    QMetaType::Void, QMetaType::Bool,    5,
    QMetaType::Void, QMetaType::Bool,    5,

       0        // eod
};

void CustomMessageBox::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        CustomMessageBox *_t = static_cast<CustomMessageBox *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->SendCurType((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 1: _t->OnSelectDecArea((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 2: _t->OnSelectTriLine((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 3: _t->OnSelectEdit((*reinterpret_cast< bool(*)>(_a[1]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (CustomMessageBox::*_t)(int );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&CustomMessageBox::SendCurType)) {
                *result = 0;
                return;
            }
        }
    }
}

const QMetaObject CustomMessageBox::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_CustomMessageBox.data,
      qt_meta_data_CustomMessageBox,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *CustomMessageBox::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *CustomMessageBox::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_CustomMessageBox.stringdata0))
        return static_cast<void*>(const_cast< CustomMessageBox*>(this));
    return QDialog::qt_metacast(_clname);
}

int CustomMessageBox::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}

// SIGNAL 0
void CustomMessageBox::SendCurType(int _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
